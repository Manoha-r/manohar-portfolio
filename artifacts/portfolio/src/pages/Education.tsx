...
// 3D UNIQUE SCENE: Solar System — planets orbit outward from center on entry
function SolarSystem() {
  const groupRef = useRef<THREE.Group>(null);
  const progress = useRef(0);

  const planets = [
    { finalRadius: 2.5, speed: 0.8, size: 0.25, color: "#0071E3", yOffset: 0.3 },
    { finalRadius: 3.8, speed: 0.5, size: 0.35, color: "#4499ff", yOffset: -0.4 },
    { finalRadius: 5.2, speed: 0.3, size: 0.2, color: "#ffffff", yOffset: 0.6 },
    { finalRadius: 6.5, speed: 0.2, size: 0.3, color: "#003d99", yOffset: -0.2 },
  ];

  const planetRefs = useRef<Array<THREE.Mesh | null>>([]);
  ...
  useFrame(({ clock }, delta) => {
    ...
    planets.forEach((p, i) => {
      ...
      const localP = Math.max(0, (progress.current - i * 0.12) / (1 - i * 0.12));
      const localEase = 1 - Math.pow(1 - Math.min(localP, 1), 3);
      const currentRadius = p.finalRadius * localEase;
      const angle = t * p.speed + (i * Math.PI / 2);

      if (mesh) {
        mesh.position.set(Math.cos(angle) * currentRadius, p.yOffset, Math.sin(angle) * currentRadius);
        mesh.scale.setScalar(localEase);
      }
      if (ring) ring.scale.setScalar(localEase);
    });
  });
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* Orbit rings */}
      {planets.map((p, i) => (
        <mesh key={`ring-${i}`} rotation={[Math.PI / 2, 0, 0]} scale={0}
          ref={(el) => { ringRefs.current[i] = el as THREE.Mesh | null; }}>
          <torusGeometry args={[p.finalRadius, 0.02, 8, 100]} />
          <meshStandardMaterial color="#0071E3" emissive="#0071E3" emissiveIntensity={0.2} transparent opacity={0.25} />
        </mesh>
      ))}
      {/* Planets */}
      {planets.map((p, i) => (
        <Sphere key={`planet-${i}`} args={[p.size, 20, 20]} position={[0, p.yOffset, 0]} scale={0}
          ref={(el) => { planetRefs.current[i] = el as THREE.Mesh | null; }}>
...
// 3D SCENE: Asteroid belt of orbiting dust particles outside the solar system
function AsteroidBeltParticles() {
  const groupRef = useRef<THREE.Group>(null);
  const progress = useRef(0);

  const particles = useMemo(() => {
    return Array.from({ length: 150 }, (_, i) => {
      // Outer belt radius 7.5 to 10
      const radius = 7.5 + Math.random() * 2.5;
...
export function EducationScene() {
  return (
    <>
      ...
      <pointLight position={[0, 0, 6]} intensity={3} color="#0071E3" />
      <pointLight position={[6, 6, 0]} intensity={1} color="#4499ff" />
      <Stars radius={120} depth={60} count={4000} factor={4} saturation={0} fade speed={0.6} />
      <SolarSystem />
      <AsteroidBeltParticles />
